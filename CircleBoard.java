...
public class CircleBoard implements ActionListener, KeyListener{
    ...
    */
    ...
    CircleBoard() {
        ...
        for(int i = 0; i < HEIGHT; i++) {
            for(int j = 0; j < LENGTH; j++) {
                ...
                int k = LENGTH/2;
                int h = HEIGHT/2;
                if(((double) (j-k)*(j-k))/((double) k*k) + ((double) (i-h)*(i-h))/((double) h*h) > 1) {
                    spots[i][j] = 7;
        ...
        while(spots[10][2] == spots[2][10]) {
            int tile = rand.nextInt(6);
            spots[10][2] = tile;
            panels[10][2].setBackground(colors[tile]);
            
        }
        
        player1Color = spots[10][2];
        player2Color = spots[2][10];
    ...
    public void checkGameOver() {
        if(player1Score + player2Score != 113) return;
